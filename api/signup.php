...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)
    ...
    apiResponse(422, ['ok' => false, 'message' => 'Datos de registro inválidos']);
}

$duplicate = $pdo->prepare('SELECT id FROM users WHERE email = ? OR username = ? LIMIT 1');
$duplicate->execute([$email, $username]);
if ($duplicate->fetch()) {
    apiResponse(409, ['ok' => false, 'message' => 'El correo o usuario ya está registrado']);
}
...
$statement->execute([
    $email, $fullName, $username, password_hash($password, PASSWORD_DEFAULT),
    $age, $sex, $allergies !== '' ? $allergies : 'Ninguna',
]);
$userId = (int) $pdo->lastInsertId();
$userStatement = $pdo->prepare(
    'SELECT id, email, full_name, username, age, sex, allergies FROM users WHERE id = ?'
);
