...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)
    ...
    apiResponse(422, ['ok' => false, 'message' => 'Datos de perfil inválidos']);
}
...
if ($password !== '') {
    $statement = $pdo->prepare(
        'UPDATE users SET email=?, full_name=?, username=?, age=?, sex=?, allergies=?, password=?
         WHERE id=?'
    );
    $statement->execute([
    ...
    $statement->execute([
        $email, $fullName, $username, $age, $sex, $allergies ?: 'Ninguna', $user['id'],
    ]);
}
